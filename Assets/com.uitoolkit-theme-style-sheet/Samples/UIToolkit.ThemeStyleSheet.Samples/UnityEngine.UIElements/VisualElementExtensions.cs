...
namespace UnityEngine.UIElements {
    ...
    public static partial class VisualElementExtensions {
        ...
        public static T RequireElement<T>(this VisualElement element, string? name, params string[] classes) where T : VisualElement {
            ...
            if (result == null) throw new InvalidOperationException( $"Element {typeof( T )} ({name}, {classes}) was not found" );
            return result;
        }
        [MethodImpl( MethodImplOptions.AggressiveInlining )]
        public static T[] RequireElements<T>(this VisualElement element, string? name, params string[] classes) where T : VisualElement {
            var result = element.Query<T>( name, classes ).ToList();
            if (result.Count == 0) throw new InvalidOperationException( $"Elements {typeof( T )} ({name}, {classes}) was not found" );
